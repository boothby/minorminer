...
class cell_cache {
  ...
  public:
    ...
    cell_cache(const topo_spec p, const vector<size_t> &nodes,
               ...
               borrow(false), topo(p),
               nodemask(new shore_t[p.num_cells()]{}),
               edgemask(new shore_t[p.num_cells()]{}) {
        topo.process_nodes(nodemask, edgemask, nodes);
        topo.process_edges(edgemask, edges);
    }

    cell_cache(const topo_spec p, shore_t *nm, shore_t *em) :
               borrow(true), topo(p), nodemask(nm), edgemask(em) {}

  public:
    shore_t qmask(bool u, size_w w, size_z z) const {
        return nodemask[topo.cell_index(u, w, z)];
    }
    shore_t emask(bool u, size_w w, size_z z) const {
        return edgemask[topo.cell_index(u, w, z)];
    }

    shore_t score(size_y y, size_x x) const {
    ...
    void inflate(size_y y, size_x x, vector<vector<size_t>> &emb) const {
        shore_t k0 = qmask(0, vert(x), vert(y));
        shore_t k1 = qmask(1, horz(y), horz(x));
        while (k0 && k1) {
            emb.emplace_back(0);
