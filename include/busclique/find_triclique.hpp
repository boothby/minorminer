...
void find_triclique_raw(topo_cache<topo_spec> &topology,
                  size_t size_c,
                  size_t bip_v,
                  size_t bip_h,
                  vector<vector<size_t>> &emb) {
    bundle_cache<topo_spec> bundles(topology.cells);
    clique_cache<topo_spec> rects(topology.cells, bundles, size_c+bip_v+bip_h, bip_v, bip_h);
    rects.extract_solution(emb);
}

/*
template<typename cells_t>
bool find_triclique_short(const cells_t &cells, const size_t size, vector<vector<size_t>> &emb, size_t &max_length) {
...
template<typename topo_spec>
bool find_triclique_nice(const cell_cache<topo_spec> &,
                      size_t size,
                      size_t bip_v,
                      size_t bip_h,
                      vector<vector<size_t>> &emb,
...
template<>
bool find_triclique_nice(const cell_cache<chimera_spec> &cells,
                      size_t size,
                      size_t bip_v,
                      size_t bip_h,
                      vector<vector<size_t>> &emb,
    ...
    if(size <= shore && find_triclique_short(cells, size, emb, max_length))
        return true;
    ...
    for(size_t width = minw; width <= maxw; width++) {
        clique_cache<chimera_spec> rects(cells, bundles, width, bip_v, bip_h);
...
template<>
bool find_triclique_nice(const cell_cache<zephyr_spec> &cells,
                      size_t size,
                      size_t bip_v,
                      size_t bip_h,
                      vector<vector<size_t>> &emb,
    ...
    for(size_t width = minw; width <= maxw; width++) {
        clique_cache<zephyr_spec> rects(cells, bundles, width, bip_v, bip_h);
        vector<vector<size_t>> temp_emb;
        size_t l = get_sol(rects, temp_emb, size);
        if (l < max_length) {
            max_length = l;
...
template<>
bool find_triclique_nice(const cell_cache<pegasus_spec> &cells,
                      size_t size,
                      size_t bip_v,
                      size_t bip_h,
                      vector<vector<size_t>> &emb,
    ...
    for(size_t w = minw; w <= maxw; w++) {
        ...
        if (w_lengthmin < max_length) {
            ...
            clique_cache<pegasus_spec> rects(cells, bundles, w, bip_v, bip_h);
        ...
        for (size_t length = w_lengthmin; length < w_lengthmax; length++) {
            ...
            clique_cache<pegasus_spec> rects(cells, bundles, w, bip_v, bip_h, check_length);
            vector<vector<size_t>> tmp;
...
template<typename topo_spec>
bool find_triclique(const topo_spec &topo,
    ...
    return find_triclique(topology, size, bip_v, bip_h, emb);
}

template<typename topo_spec>
bool find_triclique(topo_cache<topo_spec> &topology,
                 size_t size,
                 size_t bip_v,
                 size_t bip_h,
                 vector<vector<size_t>> &emb) {
    ...
    if (find_triclique_nice(topology.cells, size, bip_v, bip_h, _emb, min_width, max_width, max_length))
        emb = _emb;
    while(topology.next()) {
        if (find_triclique_nice(topology.cells, size, bip_v, bip_h, _emb, min_width, max_width, max_length))
            emb = _emb;
...
template<typename topo_spec>
void short_triclique(const topo_spec &,
                  const vector<size_t> &nodes,
                  const vector<pair<size_t, size_t>> &edges,
                  vector<vector<size_t>> &emb) {
...
template<typename topo_spec>
void best_tricliques(topo_cache<topo_spec> &topology,
